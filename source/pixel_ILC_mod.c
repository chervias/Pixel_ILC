...
static PyObject *doNILC(PyObject *self, PyObject *args){
	/* Getting the elements */
	// TQUmaps will be a numpy array with the shape [Nwindows,Nfreqs,3,npix]
	// ipix_arr will be the array with all the pixel indices
	//a will be an array with shape [Nfreqs] which contains the CMB SED (in RJ units)
	...
	PyObject *ipix_arr=NULL;
	PyObject *Npixels=NULL;
	PyObject *Nwindows=NULL;
	PyObject *rank=NULL;
	
	if (!PyArg_ParseTuple(args, "OOOOOOOOO",&TQUmaps, &nside, &a, &fwhm, &Nfreqs, &ipix_arr, &Npixels, &Nwindows, &rank))
		return NULL;
	...
	unsigned long Npixels_ = (long) PyLong_AsLong(Npixels);
	unsigned int rank_ = (int) PyLong_AsLong(rank);
	unsigned int Nwindows_ = (int) PyLong_AsLong(Nwindows);

	double fwhm_ = PyFloat_AsDouble(fwhm);
	double sigma = fwhm_ / (2.0*sqrt(2*log(2.0)));
	
	double* weights = calloc(Npixels_*Nwindows_*3*Nfreqs_,sizeof(double));
	...
	for(n=0;n<Nwindows_;n++){
		for(p=0;p<Npixels_;p++){
			if(p%5000==0){
				printf("Rank %i is working on pixel number %i of %i for needlet window %i of %i \n",rank_,p,Npixels_,n+1,Nwindows_);
			}
			// This is the index of the pixel to process
			ipix = (*(long*)PyArray_GETPTR1(ipix_arr,p));
			//if(p==0){
			...
			empty_mat_contents(CovQi,Nfreqs_);
			empty_mat_contents(CovUi,Nfreqs_);
			pixelILC_DefineCovarianceMatrix_NILC(ipix, n, nside_map, Nfreqs_, TQUmaps, sigma, CovT, CovQ, CovU);
			// Now we need to invert the Cov matrices
			invert_a_matrix(CovT,CovTi,Nfreqs_);
			invert_a_matrix(CovQ,CovQi,Nfreqs_);
			invert_a_matrix(CovU,CovUi,Nfreqs_);
			pixelILC_CalculateILCWeight_NILC(a,CovTi,CovQi,CovUi,weights,Nfreqs_,Nwindows_,p,n);
		}
	}
	gsl_matrix_free(CovT);
	gsl_matrix_free(CovQ);
	...
	npy_intp npy_shape[4] = {Npixels_,Nwindows_,3,Nfreqs_};
	PyObject *arr 		= PyArray_SimpleNewFromData(4,npy_shape, NPY_DOUBLE, weights);
	PyArray_ENABLEFLAGS((PyArrayObject *)arr, NPY_OWNDATA);
	return(arr);
}
...
static PyMethodDef PixelILCMethods[] = {
  ...
  {"doNILC",  doNILC, METH_VARARGS,NULL},
